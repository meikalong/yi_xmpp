...
public class StanzaHandler {
	...
	public void process(String stanza, XMPPPacketReader reader) throws Exception {
		...
		// Create DOM object
		Document document = reader.read(new StringReader(stanza));
	...
	private void processChat(Element root) {
		...
		try {
			...
			if (!root.elements().isEmpty()) {
				reply.setChildElement(((Element) root.elements().get(0)).createCopy());
			}
			reply.setID(root.attributeValue("id"));
